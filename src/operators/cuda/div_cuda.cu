...
__global__ void div_kernel_vectorized(
    const float* a,
    const float* b,
    float* out,
    int64_t n
...
__global__ void div_kernel_simple(
    const float* a,
    const float* b,
    float* out,
    int64_t n
...
/**
 * @brief Div CUDA Plugin
 */
class DivCUDAPlugin : public SimpleCUDAPlugin<DivCUDAPlugin> {
public:
    DivCUDAPlugin() = default;
    ~DivCUDAPlugin() override = default;
    ...
    core::Status enqueue(
        ...
        if (!input_a || !input_b || !output || !context.device_context) {
            return core::Status::ERROR_INVALID_ARGUMENT;
        }
        ...
        if (shape_a == shape_b) {
            if (total < 1024) {
                ...
                div_kernel_simple<<<blocks, threads, 0, cuda_ctx->stream()>>>(
                    data_a, data_b, data_out, total
                );
            } else {
                int blocks = std::min((total + threads * 4 - 1) / (threads * 4), (int64_t)2048);
                div_kernel_vectorized<<<blocks, threads, 0, cuda_ctx->stream()>>>(
                    data_a, data_b, data_out, total
                );
            }
        } else {
...
REGISTER_PLUGIN_SIMPLE(DivCUDAPlugin, "Div", kDIV, CUDA)
