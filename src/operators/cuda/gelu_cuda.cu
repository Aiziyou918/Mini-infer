...
__global__ void gelu_kernel_vectorized(
    ...
    for (int64_t i = tid; i < vec_size; i += stride) {
        ...
        float4 out;
        out.x = gelu_tanh(val.x);
        out.y = gelu_tanh(val.y);
        out.z = gelu_tanh(val.z);
        out.w = gelu_tanh(val.w);
        output_vec[i] = out;
    }
...
__global__ void gelu_kernel_simple(
    const float* input,
    float* output,
    int64_t n
) {
...
/**
 * @brief GELU CUDA Plugin
 *
 * Implements Gaussian Error Linear Unit activation using CUDA.
 * Uses the tanh approximation for better performance.
 */
class GELUCUDAPlugin : public SimpleCUDAPlugin<GELUCUDAPlugin> {
public:
    GELUCUDAPlugin() = default;
    ~GELUCUDAPlugin() override = default;
    ...
    core::Status enqueue(
        ...
        if (n < 1024) {
            ...
            gelu_kernel_simple<<<blocks, threads, 0, cuda_ctx->stream()>>>(
            ...
            gelu_kernel_vectorized<<<blocks, threads, 0, cuda_ctx->stream()>>>(
                static_cast<const float*>(input->data()),
                static_cast<float*>(output->data()),
                n
            );
...
REGISTER_PLUGIN_SIMPLE(GELUCUDAPlugin, "Gelu", kGELU, CUDA)
