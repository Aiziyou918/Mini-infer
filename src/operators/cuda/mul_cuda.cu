...
__global__ void mul_kernel_vectorized(
    const float* a,
    const float* b,
    float* out,
    int64_t n
...
__global__ void mul_kernel_simple(
    const float* a,
    const float* b,
    float* out,
    int64_t n
...
/**
 * @brief Mul CUDA Plugin
 */
class MulCUDAPlugin : public SimpleCUDAPlugin<MulCUDAPlugin> {
public:
    MulCUDAPlugin() = default;
    ~MulCUDAPlugin() override = default;
    ...
    core::Status enqueue(
        ...
        if (shape_a == shape_b) {
            if (total < 1024) {
                ...
                mul_kernel_simple<<<blocks, threads, 0, cuda_ctx->stream()>>>(
                    data_a, data_b, data_out, total
                );
            } else {
                int blocks = std::min((total + threads * 4 - 1) / (threads * 4), (int64_t)2048);
                mul_kernel_vectorized<<<blocks, threads, 0, cuda_ctx->stream()>>>(
                    data_a, data_b, data_out, total
                );
            }
        } else {
...
REGISTER_PLUGIN_SIMPLE(MulCUDAPlugin, "Mul", kMUL, CUDA)
