...
class TanhCUDAPlugin : public SimpleCUDAPlugin<TanhCUDAPlugin> {
public:
    TanhCUDAPlugin() = default;
    ~TanhCUDAPlugin() override = default;
    ...
    core::Status enqueue(
        ...
        if (total < 1024) {
            ...
            tanh_kernel<<<blocks, threads, 0, cuda_ctx->stream()>>>(
                data_in, data_out, total
            );
        } else {
            int blocks = std::min((total + threads * 4 - 1) / (threads * 4), (int64_t)2048);
            tanh_kernel_vectorized<<<blocks, threads, 0, cuda_ctx->stream()>>>(
                data_in, data_out, total
            );
        }
...
REGISTER_PLUGIN_SIMPLE(TanhCUDAPlugin, "Tanh", kTANH, CUDA)
