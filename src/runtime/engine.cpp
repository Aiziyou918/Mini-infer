...
core::Status Engine::infer_shapes() {
    ...
    for (auto& node : sorted_nodes_) {
        ...
        if (output_shapes.empty()) {
            MI_LOG_ERROR("[Engine] Node " + node->name() + " produced empty output shapes");
            return core::Status::ERROR_RUNTIME;  // Fail fast: stop on first error
        }
        ...
        for (size_t i = 0; i < output_shapes.size() && i < output_tensors.size(); ++i) {
            ...
            if (config_.enable_profiling) {
                MI_LOG_INFO("[Engine] Node " + node->name() + " output[" + std::to_string(i) +
                            "] shape: " + output_shapes[i].to_string());
            }
        }
    }

    MI_LOG_INFO("[Engine] Shape inference completed: " + std::to_string(total_inferred) +
                " tensor(s) inferred");
...
core::Status Engine::infer_shapes_with_profile() {
    ...
    if (config_.enable_profiling) {
        MI_LOG_INFO("[Engine] Using max profile with optimal shapes:");
    ...
    for (const auto& input_name : graph_->inputs()) {
        ...
        if (it == max_shapes.end()) {
            MI_LOG_WARNING("[Engine] No max shape for input '" + input_name + "'");
            continue;
        }

        auto node = graph_->get_node(input_name);
        if (node) {
            // Create or update input tensor with max shape
    ...
    // Now perform normal shape inference (will use the max input shapes)
    return infer_shapes();
}
...
core::Status Engine::update_tensor_properties() {
    ...
    for (const auto& node : sorted_nodes_) {
        ...
        for (size_t idx = 0; idx < outputs.size(); ++idx) {
            ...
            if (config_.enable_profiling) {
                MI_LOG_INFO("[Engine] Tensor '" + node->name() + "' output[" + std::to_string(idx) +
    ...
    MI_LOG_INFO("[Engine] Updated metadata for " + std::to_string(updated_count) + " tensor(s)");
    return core::Status::SUCCESS;
}
...
core::Status Engine::plan_memory() {
    ...
    MI_LOG_INFO("[Engine]   Number of pools:  " + std::to_string(memory_plan_.pools.size()));
...
core::Status Engine::allocate_tensors() {
    ...
    const bool use_memory_pools = config_.enable_memory_planning && !memory_plan_.pools.empty();
    ...
    MI_LOG_INFO("[Engine] Tensor allocation completed: " + std::to_string(allocated_count) +
                " allocated, " + std::to_string(skipped_count) + " skipped, " +
...
core::Status Engine::prepare_memory_pools(bool use_memory_pools) {
    ...
    if (!use_memory_pools) {
        if (config_.enable_memory_planning && config_.enable_profiling) {
            MI_LOG_WARNING(
                "[Engine] Memory planning enabled but no pools available; "
    ...
    for (const auto& pool : memory_plan_.pools) {
        ...
        if (!raw && pool.size_bytes > 0) {
            MI_LOG_ERROR("[Engine] Failed to allocate memory pool " + std::to_string(pool.pool_id) +
                         " of size " + std::to_string(pool.size_bytes) + " bytes");
            return core::Status::ERROR_RUNTIME;
        }
...
core::Status Engine::allocate_node_outputs(std::shared_ptr<graph::Node> node, bool use_memory_pools,
    ...
    for (size_t i = 0; i < output_tensors.size(); ++i) {
        ...
        if (!tensor) {
            if (config_.enable_profiling) {
                MI_LOG_WARNING("[Engine] Node " + node->name() + " output[" + std::to_string(i) +
                               "] is null");
            }
            failed_count++;
            continue;
        ...
        if (shape.ndim() == 0) {
            if (config_.enable_profiling) {
                MI_LOG_WARNING("[Engine] Node " + node->name() + " output[" + std::to_string(i) +
                               "] has empty shape, skipping allocation");
            }
            failed_count++;
            continue;
        }
        ...
        if (shape.numel() <= 0) {
            MI_LOG_ERROR("[Engine] Node " + node->name() + " output[" + std::to_string(i) +
                         "] has invalid shape: " + shape.to_string());
            failed_count++;
            continue;
        }

        const auto bind_result = try_bind_tensor_to_pool(node->name(), i, tensor, use_memory_pools,
        ...
        try {
            *tensor = core::Tensor(shape, tensor->dtype());
            ...
            if (config_.enable_profiling) {
                MI_LOG_INFO("[Engine] Allocated tensor for " + node->name() + " output[" +
                            std::to_string(i) + "]: " + shape.to_string() + " (" +
                            std::to_string(tensor->size_in_bytes() / 1024.0) + " KB)");
            }
        } catch (const std::exception& e) {
            MI_LOG_ERROR("[Engine] Failed to allocate tensor for " + node->name() + " output[" +
...
Engine::PoolBindResult Engine::try_bind_tensor_to_pool(const std::string& tensor_name,
                                                       size_t output_index,
    ...
    const bool valid_pool = pool_id >= 0 &&
    ...
    if (required > pool_size) {
        MI_LOG_ERROR("[Engine] Tensor " + tensor_name + " output[" + std::to_string(output_index) +
                     "] requires " + std::to_string(required) + " bytes, but pool " +
...
core::Status Engine::handle_shape_change(
    ...
    if (!tensors_to_reallocate.empty()) {
        MI_LOG_INFO("[Engine] Resizing " + std::to_string(tensors_to_reallocate.size()) +
                    " tensor(s) due to shape change");

        // Resize tensors with new shapes (in-place, preserving pointers)
        for (const auto& tensor_name : tensors_to_reallocate) {
            ...
            // Resize existing tensor instead of creating new one
            if (!node->output_tensors().empty() && node->output_tensors()[0]) {
                node->output_tensors()[0]->resize(*new_shape);

                if (config_.enable_profiling) {
                    MI_LOG_INFO("[Engine]   Resized '" + tensor_name +
                                "': " + new_shape->to_string());
                }
            }
        }
